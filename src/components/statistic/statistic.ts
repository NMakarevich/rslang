...
class Statistic {
  async draw() {
    ...
    if (localStorageUtil.checkAuthorization()) {
      ...
      const sprintPercent = await getPercent('sprint');
      const sprintSeries = await getSeries('sprint');
      for (let i = 0; i < 2; i += 1) {
        ...
        if (i === 0) {
          ...
          // if (localStorageUtil.checkAuthorization()) {
          statisticCountText.innerHTML = sprintPercent;
          // }
        }
        if (i === 1) {
          statisticCountName.innerHTML = 'Самая длинная серия';
          // if (localStorageUtil.checkAuthorization()) {
          statisticCountText.innerHTML = String(sprintSeries);
          // }
        }
        // if (i === 2) {
      ...
      const audiocallPersent = await getPercent('audiocall');
      const audiocallSeries = await getSeries('audiocall');
      for (let i = 0; i < 2; i += 1) {
        const statisticItem = document.createElement('div');
        statisticItem.classList.add('statistic__item');
        const statisticCount = document.createElement('div');
        ...
        if (i === 0) {
          statisticCountName.innerHTML = '% правильных ответов';
          statisticCountText.innerHTML = audiocallPersent;
        }
        if (i === 1) {
          statisticCountName.innerHTML = 'Самая длинная серия';
          statisticCountText.innerHTML = String(audiocallSeries);
        }
        // if (i === 2) {
        //   statisticCountName.innerHTML = 'Новые слова за день';
        // }
    ...
    staticticWrapper.innerHTML = 'Зарегистрируйтесь, чтобы получить статистику';
    return staticticWrapper;
  }
}
